...
- Claude Code state machine: needs-you comes ONLY from the Notification hook (`~/.claude/twinagent-notify.jsonl`, installed in both sides' settings.json) — a pending tool is just tool-running however long it takes (the old >2.5s heuristic false-alarmed on every long command). The **PreToolUse hook** appends a resolve line to the same file: it fires on approval (before the tool runs), which clears needs-you so an approved long tool isn't stuck red for its whole duration. WSL's PreToolUse uses a `sed` that emits only `{session_id, hook_event_name}` (jq absent, and it fires per tool call); the file is truncated on collector boot if >1MB. `stop_reason == "end_turn"` → done; ~10s silence → idle; result containing "rejected" → interrupted; done/needs-you never decay to stale.
